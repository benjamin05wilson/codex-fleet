...
Indexing is bounded to 500 eligible text files, 80 KB per file, 4 MB per workspace and 20 registered worktrees per pass. Notes report coverage exclusions; large notes are capped. Secret paths, credentials, symlinks, binary files, dependency folders and generated build outputs are excluded, and retained text is redacted. These safeguards are best-effort, not a guarantee that arbitrary source contains no sensitive information. Everything stays in the local Fleet data directory. This is evidence-backed static indexing, not an autonomous model interpreting every keystroke.
...
Context selection ranks local notes against the task, uses a 12,000-character budget, and excludes stale notes and unapproved proposals. A session receives shared decisions plus its own worktree's current code knowledge; other worktree layers are excluded, and current worktree topics supersede their project equivalents. Pinning does not bypass worktree isolation. Pin or exclude notes using the Brain's Context menu. Session details show the exact stored context selection used for the run.
