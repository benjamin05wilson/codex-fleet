...
In Fleet Desktop, choose **Tools → Browser**, enter a website or local preview URL and click **Open browser**. Fleet renders Chromium directly inside the app at the screen’s native pixel density, with in-memory caching. This is the only browser: there is no streamed Chrome/headless mode, selector or fallback. Web clients and older desktop builds show an update/desktop requirement instead of launching another browser.

The browser is **manual-only**. Chat browser control is not connected yet; the daemon does not launch a browser or grant a browser connection to new chat turns. Retired browser-control and sharing APIs return HTTP 410, and chats explain the limitation rather than suggesting the removed sharing workflow. This release does not provide screenshot/evidence capture, multiple tabs, downloads or permission prompts.

Each native browser has its own temporary in-memory session. Closing it, switching projects or unmounting the browser clears that session, including cookies, cache and unsaved page state. Closing returns to the native start form, not another browser. Public HTTP(S) navigation is supported; the DNS-pinning proxy blocks unrelated private-network destinations and Fleet’s internal ports. An explicitly selected loopback preview grants only its initial local origin. Remote pages have no preload, Node or privileged IPC access. Pop-ups, downloads and permission prompts remain disabled. This does not guarantee site compatibility or challenge-free authentication.

Run `node tests/live-native-browser.mjs --run --ui` for an isolated test of the actual project browser, production preload, native rendering, resize, close confirmation and return to native setup. `--run` checks native input, scrolling, session isolation, network restrictions and cache reuse/cleanup. Optional `--run --public --scroll-profile --repeat-scroll` diagnoses Shopify in a separate profile, without model calls or access to the user's current page. See [browser measurements and historical experiments](BROWSER-MODES.md). The earlier Chrome controller tests remain historical test fixtures, not product fallback modes.
