...
Fleet Desktop opens `WebContentsView` from **Tools → Browser** or automatically when a project agent calls `navigate`. The desktop opener creates the native page if needed and reveals the Browser panel in the requesting chat; no initial click is required. There is no Chrome/headless selector, streamed view or fallback. The user and project coding agents share that same page automatically, with no connect/takeover or per-action Fleet approval. Switching chat tools hides the surface and retains the page and agent connection. Explicit close, opening another project's browser in the same window, or quitting clears the temporary session. Fleet Desktop must be running; the daemon cannot render a native page on its own.

The main process registers its page through a CSRF-authenticated local transport; its private capability does not reach websites, the renderer or model prompt. Long-poll command delivery wakes immediately without interval polling latency. Each coding chat turn receives its own capability bound to project, run and worker identity. Completed/paused/deleted/replaced attempts cannot dispatch new actions; queued actions are revalidated. Multiple agent commands are serialized while native user input stays enabled. A disconnected or timed-out command is not replayed automatically: an in-flight action may already have completed. Duplicate project windows are rejected rather than choosing an ambiguous page. Managed read-only reviewers receive no browser connection.

The desktop also maintains an authenticated opener receiver while no page exists. A `navigate` request derives its project and chat from the worker capability, prepares or reuses the native view, reveals the matching chat's panel, then dispatches one navigation through the page connection. Opener commands use the same bounded queue, turn revalidation and disconnect handling. Other actions cannot reopen a closed page with stale references. `/api/state` reports `browserAutoOpenAvailable` when the desktop opener is connected. The regression command `node tests/live-native-browser.mjs --run --ui --shared --auto-open` exercises cold opening through real stdio MCP with no manual click or model call.

Agent operations are finite and validated: navigate/back/forward/reload, snapshot/screenshot, click/fill/press/scroll. DOM operations run application-owned code in an isolated JavaScript world; agent input is data, not JavaScript or selectors. Element references are invalidated by new snapshots, navigation, removed/relabelled targets or obscuring overlays. Keyboard events use the page's private Electron debugger, not an externally listening CDP port. The page keeps its normal native compositor and pixel density. Main-document controls are supported; iframe/closed-shadow controls, password/file inputs, new tabs, downloads and device permission prompts are not automated. Internal/private-network protections remain; disabling approval prompts is not an OS sandbox or permission for page content to authorize unrelated actions.

Checks: `npm run check`; `node tests/live-native-browser.mjs --run --ui --shared` passed real stdio MCP → daemon → native page navigation/click/fill/keys/scroll/screenshot alongside human input, stale-reference rejection, internal/file/eval rejection, resize and explicit close. `--run --shared` also passed native scrolling and cache/session cleanup after awaiting OS focus in the fixture. Two earlier runs timed out before the agent checks on simulated user scrolling; the fixture now waits for actual window/page focus before dispatching that input. Tests are isolated and make no model calls. `node tests/live-browser-tools.mjs --run` verified installed Codex tool discovery at worker startup and on a new thread. A no-turn ephemeral thread could not be resumed because it has no persisted rollout; persisted-thread resumption remains fixture-tested, with MCP settings supplied at worker startup as well as per-thread. These settings use the documented [automatic MCP approval policy](https://learn.chatgpt.com/docs/extend/mcp?surface=cli). Applying the build to an already-running desktop/service requires a restart.

Retired Chrome start/control/share/frame routes still return HTTP 410. The native agent endpoint accepts only valid attempt capabilities, and the legacy binary remains absent from the app. Historical controller code/tests are reference fixtures, not runtime fallback modes.
