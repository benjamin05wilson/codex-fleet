...
Choose **Standard** (recommended and selected by default) or explicitly acknowledge **YOLO**. Standard automatically edits project files and runs sandboxed commands, with command network access disabled. Read-only is available in conversation settings and remains the mode for reviewers. Existing saved read-only preferences are not migrated automatically. YOLO uses Codex's `danger-full-access` sandbox setting and `never` approval policy on new and resumed turns, matching the documented [full-access mode](https://learn.chatgpt.com/docs/agent-approvals-security). It can access the network and modify files outside the project. A worktree is not a security boundary. YOLO runs can execute alongside Fleet agents, shells and previews in other working folders, subject to the normal concurrency limit. Fleet still prevents overlapping agent turns in the same working folder, while chat can coexist with an open shell; these checks do not constrain access to other folders or applications. Managed team/workflow tasks cannot use YOLO; reviewers stay read-only. Existing OS and administrator restrictions still apply. Fleet's per-turn instructions match the selected permission mode: YOLO permits task-authorized software installation and work outside the starting folder, while Standard and Read only retain their boundaries. Each turn supersedes older Fleet-generated permission instructions, including after a mode change.
