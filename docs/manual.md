...
Choose **Standard** (recommended and selected by default) or explicitly acknowledge **YOLO**. Standard automatically edits project files and runs sandboxed commands, with command network access disabled. Read-only is available in conversation settings and is the initial default for reviewers. Existing saved read-only preferences are not migrated automatically. YOLO uses Codex's `danger-full-access` sandbox setting and `never` approval policy on new and resumed turns, matching the documented [full-access mode](https://learn.chatgpt.com/docs/agent-approvals-security). It can access the network and modify files outside the project. A worktree is not a security boundary. YOLO runs can execute alongside Fleet agents, shells and previews in other working folders, subject to the normal concurrency limit. Fleet still prevents overlapping agent turns in the same working folder, while chat can coexist with an open shell or preview; these checks do not constrain access to other folders or applications. Managed team, workflow, mission and review sessions can also use explicitly acknowledged YOLO. Idle conversation settings remain available with an open shell or preview. Existing OS and administrator restrictions still apply. Fleet's per-turn instructions match the selected permission mode: YOLO permits task-authorized software installation and work outside the starting folder, while Standard and Read only start within their configured sandbox and can request additional permissions. Command, file-change and permission requests appear in chat with Approve and Deny controls for the current turn. Authorized commits, pushes, deployments and credential use are no longer prohibited by Fleet-generated prompts. Each turn supersedes older Fleet-generated permission instructions, including after a mode change.
...
A dead worker becomes interrupted; Fleet does not automatically repeat its potentially side-effectful work. Resume is explicit. A machine reboot does not preserve a live process. Conversation turns have no automatic time limit. Explicitly configured team assessment budgets still apply. Workers from older Fleet versions are replaced when their next turn starts; active turns are preserved across a daemon restart.

**Native Codex TUI handoff is not enabled.** The capabilities endpoint reports this explicitly. The Shell tool is a real PTY-backed **worktree shell**, not Codex's TUI and not recorded output pretending to be interactive. It runs as your local user, keeps its controls with one client, and invalidates validation. Another client cannot take its controls without the owner closing it. You can switch back to chat and send follow-ups while the shell stays open. Close the shell before running validation, requesting an independent review or accepting changes.
...
Defaults are three concurrent executions, at most five tasks, one corrective retry per task, and no automatic execution timeout. Overlapping declared write scopes are serialized. Dependencies may proceed from checked results within the same approved workflow, without human acceptance of every intermediate task. Dependency handoff copies verified changed files into a separate worktree; symbolic-link handoffs require manual review.
...
Each native browser has its own temporary in-memory session. Switching chat tools hides the surface without closing it or disconnecting agents; reopening the tool restores that same page. Explicit close, opening another project's browser in that window, or quitting Fleet clears its cookies, cache and page state. Closing returns to native setup, not another browser. HTTP(S) navigation includes local, LAN and VPN destinations. The DNS-pinning proxy still protects Fleet’s internal ports. Remote pages have no preload, Node or privileged IPC access. Downloads, pop-ups and website permission requests use normal browser behavior, subject to OS permissions. The agent supports embedded frames and closed shadow controls, password filling, file uploads using absolute paths, and tab listing, creation, switching and closing. Other available browser or desktop tools are allowed; a missing Fleet browser adapter does not block the chat turn. Snapshots omit input values. No site-compatibility or challenge-bypass guarantee is made.
