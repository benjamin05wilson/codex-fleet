...
`npm run check` runs fixture server/UI tests and a production build. Server tests have a 90-second per-test deadline; CI check/test steps also enforce a five-minute ceiling for leaked handles after test completion. `test:codex` invokes a real CLI sandbox check; `tests/live-session.mjs --run` and `tests/live-team.mjs --run` make real model turns. These are deliberately excluded from the no-account route.
